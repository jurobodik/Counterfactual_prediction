...
# ---------- 12) Plot ----------
rank_pal <- c("1 (best)"="#1a9850","2"="#91cf60","3"="#d9ef8b","4"="#fee08b","(Missing)"="gray80")
...
p_split <- ggplot(ranked, aes(x = method, y = scenario, fill = rank_cat)) +
  ...
  scale_fill_manual(values = rank_pal, name = "Rank (IS)", drop = FALSE, na.value = "gray80") +
  labs(title = "Interval Scores of different prediction intervals (Ranked)", x = NULL, y = NULL) +
...
# Save plot
ggsave("IS_heat_v2.pdf", width = 8.5, height = 6, dpi = 500)
